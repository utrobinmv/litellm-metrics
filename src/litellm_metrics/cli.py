"""litellm-metrics — CLI entry point.

Real-time консольный мониторинг LiteLLM proxy сервера.
Забирает метрики с /metrics endpoint в формате Prometheus,
парсит их и отображает в Rich Live-дашборде.

Использование:
    litellm-metrics                          # использует .env
    litellm-metrics --url http://host:port/metrics
    litellm-metrics --interval 1             # обновлять каждую секунду
    litellm-metrics --all-metrics            # показать все сырые метрики
"""
...
def parse_args():
    parser = argparse.ArgumentParser(
        prog="litellm-metrics",
        description="LiteLLM Metrics — real-time console monitoring dashboard",
    )
    parser.add_argument(
        "--url",
        help="URL метрик (по умолчанию из .env: LITELLM_METRICS_URL)",
    )
    parser.add_argument(
        "--interval",
        type=float,
...
def cli():
    ...
    console.print(f"\n[bold cyan]LiteLLM Metrics[/] — connecting to [link={metrics_url}]{metrics_url}[/]...")
